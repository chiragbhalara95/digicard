<!DOCTYPE html>
<html>
<head>
    <title>invoice#{{$orderData->order_no}}</title>
</head>
<style type="text/css">
    body{
        font-family: 'Roboto Condensed', sans-serif;
    }
    .m-0{
        margin: 0px;
    }
    .p-0{
        padding: 0px;
    }
    .pt-5{
        padding-top:5px;
    }
    .mt-10{
        margin-top:10px;
    }
    .text-center{
        text-align:center !important;
    }
    .w-100{
        width: 100%;
    }
    .w-50{
        width:50%;   
    }
    .w-85{
        width:85%;   
    }
    .w-15{
        width:15%;   
    }
    .logo img{
        width:200px;
        height:60px;        
    }
    .gray-color{
        color:#5D5D5D;
    }
    .text-bold{
        font-weight: bold;
    }
    .border{
        border:1px solid black;
    }
    table tr,th,td{
        border: 1px solid #d2d2d2;
        border-collapse:collapse;
        padding:7px 8px;
    }
    table tr th{
        background: #F4F4F4;
        font-size:15px;
    }
    table tr td{
        font-size:13px;
    }
    table{
        border-collapse:collapse;
    }
    .box-text p{
        line-height:10px;
    }
    .float-left{
        float:left;
    }
    .total-part{
        font-size:16px;
        line-height:12px;
    }
    .total-right p{
        padding-right:20px;
    }
</style>
<body>
<div class="head-title">
    <h1 class="text-center m-0 p-0">Invoice</h1>
</div>
<div class="add-detail mt-10">
    <div class="w-50 float-left mt-10">
        <p class="m-0 pt-5 text-bold w-100">Invoice Id - <span class="gray-color">#{{$orderData->order_no}}</span></p>
        <p class="m-0 pt-5 text-bold w-100">Order Id - <span class="gray-color">{{$orderData->order_no}}</span></p>
        <p class="m-0 pt-5 text-bold w-100">Order Date - <span class="gray-color">{{date("d F Y", strtotime($orderData->created_at))}}</span></p>
    </div>
    <div class="w-50 float-left logo mt-10">
        @if(!empty($companyInfoData->company_logo))
            <img src="{{url('public')}}/{{$companyInfoData->company_logo}}" alt="Logo">
        @endif
    </div>
    <div style="clear: both;"></div>
</div>
<div class="table-section bill-tbl w-100 mt-10">
    <table class="table w-100 mt-10">
        <tr>
            <th class="w-50">From</th>
            <th class="w-50">To</th>
        </tr>
        <tr>
            <td>
                <div class="box-text">
                    <p>{{$companyInfoData->company_name}},</p>
                    <p>{!!$companyInfoData->company_address!!}</p>
                    <p>Contact: {{$companyInfoData->country_code}} {{$companyInfoData->company_mobile}}, {{$companyInfoData->country_landline}}</p>
                </div>
            </td>
            <td>
                <div class="box-text">
                    <p> {{$orderData->first_name}} {{$orderData->last_name}},</p>
                    <p>{{$orderData->address}},</p>
                    <p>{{$orderData->city}}, {{$orderData->state}}</p>
                    <p>{{$orderData->zipCode}}</p>
                    <p>Contact: {{$orderData->contactNo}}</p>
                </div>
            </td>
        </tr>
    </table>
</div>
<!-- <div class="table-section bill-tbl w-100 mt-10">
    <table class="table w-100 mt-10">
        <tr>
            <th class="w-50">Payment Method</th>
            <th class="w-50">Shipping Method</th>
        </tr>
        <tr>
            <td>Cash On Delivery</td>
            <td>Free Shipping - Free Shipping</td>
        </tr>
    </table>
</div> -->

<div class="table-section bill-tbl w-100 mt-10">
    <table class="table w-100 mt-10">
        <tr>
            <th class="w-50">Product Name</th>
            <th class="w-50">Price</th>
            <th class="w-50">Qty</th>
            <th class="w-50">Subtotal</th>
            <th class="w-50">Tax Amount</th>
            <th class="w-50">Grand Total</th>
        </tr>

        @foreach($orderItemData as $orderItemDetail)
        <tr align="center">
            <td>{{$orderItemDetail->product_name}}</td>
            <td>Rs.{{$orderItemDetail->price}}</td>
            <td>{{$orderItemDetail->qty}}</td>
            <td>Rs.{{$orderItemDetail->sub_total}}</td>
            <td>Rs.{{$orderItemDetail->tax_amount1 + $orderItemDetail->tax_amount2}}</td>
            <td>Rs.{{$orderItemDetail->total}}</td>
        </tr>
        @endforeach
        <tr>
            <td colspan="7">
                <div class="total-part">
                    <div class="total-left w-85 float-left" align="right">
                        <p>Sub Total</p>
                        <p>Tax (6%)</p>
                        <p>Tax (6%)</p>
                        <p>Total Payable</p>
                    </div>
                    <div class="total-right w-15 float-left text-bold" align="right">
                        <p>Rs.{{$orderData->total - $orderData->total_tax}}</p>
                        <p>Rs.{{$orderData->total_tax1}}</p>
                        <p>Rs.{{$orderData->total_tax2}}</p>
                        <p>Rs.{{$orderData->total}}</p>
                    </div>
                    <div style="clear: both;"></div>
                </div> 
            </td>
        </tr>
    </table>
</div>
</html>
